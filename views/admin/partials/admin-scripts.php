<script>
    ...
    function renderCustomerContactSection(order) {
        ...
        // Logic template pesan & link WA tetap sama persis
    ...
    function renderPaymentSection(payment) {
        ...
        const accountInfoBlock = (isOnsite || !payment.method_account)
            ? ''
            : `<div class="mt-3 rounded-xl border border-dark-border bg-dark-base/60 p-3">
               <div class="text-[11px] text-gray-500 uppercase tracking-wider mb-1">Info Rekening / Akun</div>
               <div class="text-gold-500 font-mono text-sm break-all whitespace-pre-line leading-relaxed">${escapeHtml(payment.method_account)}</div>
           </div>`;

        // --- BLOK INFO ONSITE (CASH ON PICKUP) ---
        const onsiteInfoBlock = isOnsite
            ? `<div class="mt-3 rounded-xl border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-200">
               <div class="flex items-start gap-3">
                   ...
                   <div>
                       <div class="font-semibold text-amber-300 mb-1">Bayar di toko saat pengambilan</div>
                       <div class="text-xs text-amber-200/80 leading-relaxed">
                           Pembeli akan melunasi pesanan langsung di toko ketika datang mengambil. Konfirmasi setelah uang diterima.
                       </div>
                   </div>
               </div>
           </div>`
        ...
        const proofBlock = isOnsite
            ...
            : (proofUrl
                ? `<div class="mt-3">
                   ...
                   <button type="button" onclick="openProofViewer('${proofUrl}')" 
                       class="block w-full rounded-xl overflow-hidden border border-dark-border bg-dark-base hover:border-gold-500/50 transition text-left group">
                       <img src="${proofUrl}" alt="Bukti pembayaran" loading="lazy" 
                           class="w-full max-h-72 object-contain bg-black/40 group-hover:opacity-90 transition duration-300"/>
                       <div class="px-3 py-2 text-xs text-gray-400 flex items-center justify-between bg-dark-base relative z-10">
        ...
        // --- RENDER AKHIR ---
        return `
        <div class="bg-dark-base/50 border border-dark-border rounded-xl p-4 space-y-4">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div class="min-w-0 flex-1">
                    <div class="text-xs text-gray-500 uppercase tracking-wider mb-1">${tagLabel}</div>
                    <div class="font-semibold text-gray-100 break-words">${methodName}</div>
                    ${paidAt ? `<div class="text-xs text-gray-500 mt-1">${isOnsite ? 'Dikonfirmasi' : 'Diunggah'}: ${paidAt}</div>` : ''}
                </div>
                <div class="shrink-0 self-start sm:self-center">
                    <span class="inline-block px-3 py-1.5 rounded-full text-[11px] font-bold border ${statusInfo.cls}">
                        ${statusInfo.label}
                    </span>
                </div>
            </div>
